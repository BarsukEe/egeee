...
for i in range(len(a)):
    ...
    if 99<abs(a[i])<1000 and a[i]>minn:
        maks=a[i]
        # здесь a[i] - число из последовательности. Поскольку нам нужно найти трёхзначное число - воспользуемся модулем, чтобы отдельно не писать проверку для отрицательных трёхзначных чисел, после мы сравниваем наше трёхзначное число с самым минимальным: если наше число окажется больше - переменная maks примет значение нашего числа.
