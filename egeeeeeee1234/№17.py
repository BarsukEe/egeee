#одно из лёгких заданий, но всё ситуацию услажняет массивность кода, аспект знаний, и данные условия, которые после прочтения задания могут быть абсолютно непонятны.
...
for i in range(len(a)):
    #здесь мы перебираем абсолютно все числа. как правило, в задании нас просят вывести либо максимальное четырёхзначное, либо ещё какое число из последовательности - тобеж из всего файла. к примеру нам нужно максимальное трёхзначное число. для этого введём переменную minn, которая будет равна -100001 (как правило, в задании числа от -100000 до 100000, но бывают и исключения)
    if 99<abs(a[i])<1000 and a[i]>minn:
        maks=a[i]
        # здесь a[i] - число из последовательности. Поскольку нам нужно найти трёхзначное число - воспользуемся модулем, чтобы отдельно не писать проверку для трёхзначных чисел, после мы сравниваем наше трёхзначное число с самым минимальным: если наше число окажется больше - переменная maks примет значение нашего числа.

for i in range(len(a)-2):
    #а вот тут мы уже рассматриваем "компании" чисел. Если нам нужно работать с двойками - вписываем range(len(a)-1), если с тройками, как например здесь - range(len(a)-2). Обращаться к нашим числам мы тоже будем по другому. a[i] - первые элемент тройки, a[i+1] - второй элемент тройки, a[i+2] - третий. Представим, что нам нужно найти количество троек, в которой два числа из трёх - четырёхзначные, и их произведение больше нашего максимального трёхзначного элемента последовательности (переменная maks).
