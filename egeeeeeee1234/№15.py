...
for a in range(1,1000):
    if all(f(x)==1 for x in range(1,10000)):
        print(a)
#также, помимо x, может быть дан y, в таком случае в первую строку после x добавляем y, и в строку с условием дописываем for y in range(1,10000)
#пример:
def f(x,y):
    return выражение
...
#На числовой прямой задан отрезок P = [1315; 1415]. Обозначим через ДЕЛ(n, m) утверждение «натуральное число n делится без остатка на натуральное число m».
#Для какого наибольшего натурального A выражение
#ДЕЛ(x, A) ∨ ((x ∈ P) → (¬ДЕЛ(x, 191) ∨ (x + A ≤ 4113)))
#тождественно истинно, то есть принимает значение 1 при любом натуральном значении переменной х?

def f(x):
    return (x%a==0) or ( (x in p) <= ((x%191!=0) or (x+a<=4113)))
p = range(1315,1416)
...
#Вариации ниже вряд-ли будут на егэ.
